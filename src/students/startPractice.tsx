...
function checkIfListsEquivalent(list1,list2):boolean{
...
function strSort(strInput:string){
  return [...strInput].sort().join("")
}

function union(set1:DisjointSet, set2:DisjointSet){
...
function StartPractice(){
    ...
    function setNumAndArray(value){
      setNumPoints(value)
      const newPoints = shuffle(value).sort((a,b)=>a[0] - b[0])
      setPoints(newPoints)
      let newTriangulation = getTriangulation(setTriangulation,false,newPoints)        
      let newData = generateGraphFromTriangulation(newPoints,newTriangulation,false)
      setGraphData(newData)
      setMST(minimumSpanningTree(newData.edges,value))
    }

    const addRow = () =>{
      setNumrows(numRows + 1)
      setUserEdgesChecked(Array.from(Array(numRows + 1)))
    }
